...
export default {
	...
	methods: {
		checkRoute() {
			...
			if (id) {
				let ref = `users/pending/${ id }`;
				this.$database.get(ref)
					.then(user => {
						if (user) {
							user.id = id;
							if (this.$route.name === "signup-accept") this.requestAccepted(user);
							else if (this.$route.name === "signup-revoke") this.requestDeclined(user);
						}
					})
					.then(() => {
